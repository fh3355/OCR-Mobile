...
cv::Mat DetResizeImg(const cv::Mat img, int max_size_len,
  ...
  resize_h = resize_h > 0 ? (resize_h / 32) * 32 : 32;
  resize_w = resize_w > 0 ? (resize_w / 32) * 32 : 32;
...
DetPredictor::Postprocess(const cv::Mat srcimg,
  ...
  // FIX: Use std::vector for dynamic heap allocation instead of fixed-size stack arrays.
  // This prevents stack overflow crashes with large images.
